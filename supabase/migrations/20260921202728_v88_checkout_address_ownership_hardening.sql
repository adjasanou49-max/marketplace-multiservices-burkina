...
begin
  ...
  select nullif(trim(p_delivery_address ->> 'id'), '')::uuid
    into v_address_id;

  if v_address_id is null then
    raise exception 'delivery_address_id_required';
  end if;
  ...
  where da.id = v_address_id
    and da.customer_id = v_user;

  if v_delivery_address is null then
    raise exception 'delivery_address_not_owned';
  end if;
  ...
  loop
    ...
    if v_item.is_expirable
       and (v_item.expiry_date is null or v_item.expiry_date < current_date) then
      raise exception 'product_expired';
    end if;
    if v_item.variant_id is not null
       and v_item.variant_product_id is distinct from v_item.product_id then
    ...
    if v_coupon_id is not null
       and (
         v_coupon.seller_id is null
         or v_coupon.seller_id = (
           select s.id
           from public.sellers s
           join public.shops sh on sh.seller_id = s.id
           where sh.id = v_item.shop_id
         )
       ) then
      v_coupon_base := v_coupon_base
        + coalesce(v_item.variant_price, v_item.price) * v_item.quantity;
    end if;
  end loop;
  ...
  if v_coupon_id is not null then
    ...
    if v_coupon.discount_type = 'PERCENT' then
      v_coupon_discount := round(
        v_coupon_base * v_coupon.discount_value / 100,
        2
      );
