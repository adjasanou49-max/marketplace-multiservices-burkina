...
alter table public.coupon_usage drop constraint if exists coupon_usage_unique_order;
...
begin
  ...
  loop
    ...
    if v_seen_shop is distinct from v_item.shop_id then
      ...
      values(v_order_id,v_item.shop_id,'PAID',0)
...
alter table public.order_packages
  add constraint order_packages_weight_nonnegative check (weight_kg is null or weight_kg >= 0),
  add constraint order_packages_volume_nonnegative check (volume_m3 is null or volume_m3 >= 0);
