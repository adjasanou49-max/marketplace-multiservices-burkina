...
begin
  ...
  if p_trip_id is null or p_quantity is null or p_quantity <= 0 then
  ...
  insert into public.transport_tickets(
    booking_id, passenger_name, seat_number, status
  )
  select
    v_id,
    coalesce(v_passenger_name, 'Passager'),
    null,
    'RESERVED'
...
revoke all on function public.create_transport_booking_secure(uuid,integer,jsonb)
  from public, anon;
grant execute on function public.create_transport_booking_secure(uuid,integer,jsonb)
  to authenticated;
