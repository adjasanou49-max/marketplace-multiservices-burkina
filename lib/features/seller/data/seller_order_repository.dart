...
class SellerOrderRepository {
 ...
 Future<List<Map<String,dynamic>>> mine() async {
  ...
  final ids=(shops as List).map((e)=>(e as Map)['id']).whereType<String>().toList();
 ...
 Future<Map<String,dynamic>> detail(String groupId) async {
  ...
  final shop=await client.from('shops').select('id').eq('id',group['shop_id']).eq('seller_id',seller['id']).maybeSingle();
  if(shop==null)throw StateError('Commande inaccessible');
