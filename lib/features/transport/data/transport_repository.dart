...
class TransportRepository {
  ...
  Future<List<Map<String, dynamic>>> myBookings() async {
    ...
    final rows = await client
        ...
        .select(
          'id,trip_id,quantity,total_amount,status,created_at,'
          'transport_trips(departure_at,arrival_at,'
          'transport_routes(transport_companies(name),'
          'departure:transport_stations!transport_routes_departure_station_id_fkey(name),'
          'arrival:transport_stations!transport_routes_arrival_station_id_fkey(name)))',
        )
  ...
  Future<List<Map<String, dynamic>>> myTickets() async {
    ...
    final rows = await client
        ...
        .select(
          'id,booking_id,passenger_name,seat_number,status,created_at,'
          'transport_bookings!inner(customer_id)',
        )
