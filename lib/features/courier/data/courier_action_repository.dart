...
class CourierActionRepository {
 const CourierActionRepository(this.client);
 ...
 Future<void> accept(String assignmentId) async {
  ...
  await client.from('delivery_assignments').update({'status':'ACCEPTED','accepted_at':DateTime.now().toUtc().toIso8601String()}).eq('id',assignmentId).eq('courier_id',u.id);
 }
}