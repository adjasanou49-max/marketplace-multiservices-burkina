...
class _ServicesPageState extends ConsumerState<ServicesPage> {
  ...
  void initState() {
    ...
    categoriesFuture = repository == null ? Future.value(const []) : repository.categories();
    servicesFuture = repository == null ? Future.value(const []) : repository.services();
  }
  ...
  void _selectCategory(String? value) {
    ...
    setState(() {
      category = value;
      servicesFuture = repository == null ? Future.value(const []) : repository.services(category: value);
    });
  }
  ...
  Future<void> _requestService(Map<String, dynamic> service) async {
    ...
    final description = await showDialog<String>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text('Demander ${service['name']?.toString() ?? 'ce service'}'),
        content: TextField(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(dialogContext, controller.text.trim()), child: const Text('Envoyer')),
        ],
      ),
    );
    controller.dispose();
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            onPressed: repository == null
                ? null
                : () => setState(() {
                      categoriesFuture = repository.categories();
                      servicesFuture = repository.services(category: category);
                    }),
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, categorySnapshot) {
          final categories = categorySnapshot.data ?? const <Map<String, dynamic>>[];
          return Column(
            children: [
                    ...
                    children: [
                      ...
                      for (final item in categories) ...[
                        ChoiceChip(
                          ...
                          onSelected: (_) => _selectCategory(item['slug']?.toString()),
                        ),
                        const SizedBox(width: 8),
                      ],
                    ],
              ...
              Expanded(
                child: FutureBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (items.isEmpty) {
                      return const Center(child: Text('Aucun service disponible actuellement.'));
                    }
