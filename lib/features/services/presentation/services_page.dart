...
class _ServicesPageState extends ConsumerState<ServicesPage> {
  ...
  void initState() {
    ...
    categoriesFuture =
        repository?.categories() ?? Future.value(const []);
  ...
  Future<List<Map<String, dynamic>>> _loadServices(
    ...
    return repository.services(category: category);
  }
  ...
  Future<void> _requestService(Map<String, dynamic> service) async {
    ...
    if (repository == null || client?.auth.currentUser == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Connectez-vous pour demander un service.'),
        ),
      );
      return;
    }
    ...
    final description = await showDialog<String>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(
          'Demander ' +
              (service['name']?.toString() ?? 'ce service'),
        ),
        content: TextField(
