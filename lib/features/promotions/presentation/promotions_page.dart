...
class _PromotionsPageState extends ConsumerState<PromotionsPage> {
  ...
  Future<List<Map<String, dynamic>>> _load() async {
    ...
    final rows = await client
        .from('promotions')
        .select(
          'id,seller_id,shop_id,name,promotion_type,value,starts_at,ends_at,active,shops(name)',
        )
        .eq('active', true)
        ...
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, index) {
              ...
              final shop = item['shops'] is Map
                  ? Map<String, dynamic>.from(item['shops'] as Map)
              ...
              return Card(
                child: ListTile(
                  ...
                  title: Text(
                    item['name']?.toString() ?? 'Promotion',
                  ),
                  subtitle: Text(
                    (shop['name']?.toString() ?? 'Boutique') +
                        '\nType : ' +
                        (item['promotion_type']?.toString() ?? '-') +
                        '\nValable jusqu’au ' +
                        (item['ends_at']?.toString() ?? '-'),
                  ),
                  trailing: Text(
                    (item['value']?.toString() ?? '0') +
                        (item['promotion_type']?.toString() == 'PERCENT'
                            ? ' %'
                            : ' XOF'),
